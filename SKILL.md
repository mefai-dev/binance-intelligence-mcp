...
A Python MCP server that provides **8 computed intelligence tools** for Binance futures markets. Each tool combines 2-6 Binance API endpoints into derived analytics with composite scoring algorithms 路路路 not just raw data forwarding.
...
- `/fapi/v1/klines` 路路路 Candlestick/kline data
- `/fapi/v1/aggTrades` 路路路 Aggregate trades
- `/fapi/v1/depth` 路路路 Order book
- `/fapi/v1/premiumIndex` 路路路 Funding rate
- `/futures/data/openInterestHist` 路路路 Open interest history
- `/futures/data/topLongShortPositionRatio` 路路路 Top trader positions
- `/futures/data/topLongShortAccountRatio` 路路路 Top trader accounts
- `/futures/data/globalLongShortAccountRatio` 路路路 Global sentiment
- `/futures/data/takerlongshortRatio` 路路路 Taker buy/sell volume
...
```
MCP Client 路啋 stdio 路啋 server.py (FastMCP, 8 tools)
                        路啋 tools/*.py (scoring algorithms)
                          路啋 client.py (async aiohttp)
                            路啋 Binance Public API
```

Each tool module is a pure async function: `(client, params) 路啋 dict`. The server module wraps these with `@mcp.tool()` decorators and manages client lifecycle.
